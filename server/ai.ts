...
`;
  ...
  // ==================== HEALTH CHECK / READINESS DATA ====================

  // Health check assessments by domain (governance, raid, technical, budget_schedule, etc.)
  const healthAssessments = storage.getHealthCheckAssessments(projectId);
  if (healthAssessments.length > 0) {
    // Keep the most recent per domain
    const latestByDomain = new Map<string, typeof healthAssessments[number]>();
    for (const a of healthAssessments) {
      if (!latestByDomain.has(a.domain)) latestByDomain.set(a.domain, a);
    }
    context += `\nHEALTH CHECK ASSESSMENTS (${latestByDomain.size} domains assessed):\n`;
    for (const a of Array.from(latestByDomain.values())) {
      context += `- ${a.domain}: ${a.overallRating || "unrated"}`;
      if (a.summary) context += ` — ${a.summary.substring(0, 300)}`;
      context += `\n`;
      try {
        ...
        if (Array.isArray(findings) && findings.length > 0) {
          for (const f of findings.slice(0, 4)) {
            ...
            context += `    • ${sev}${(f.finding || "").substring(0, 160)}`;
            if (f.recommendation) context += ` → Rec: ${String(f.recommendation).substring(0, 120)}`;
            context += `\n`;
          }
          if (findings.length > 4) context += `    ... and ${findings.length - 4} more findings\n`;
        }
      } catch {}
    }
  }
  ...
  // RAID log
  const raid = storage.getRaidItems(projectId);
  if (raid.length > 0) {
    const byType: Record<string, typeof raid> = {};
    for (const r of raid) {
      const t = r.type || "other";
      if (!byType[t]) byType[t] = [];
      byType[t].push(r);
    }
    const openCount = raid.filter(r => r.status !== "closed" && r.status !== "resolved").length;
    context += `\nRAID LOG (${raid.length} entries, ${openCount} open):\n`;
    for (const [type, items] of Object.entries(byType)) {
      const open = items.filter(i => i.status !== "closed" && i.status !== "resolved");
      context += `- ${type.toUpperCase()}S (${items.length} total, ${open.length} open):\n`;
      for (const r of items.slice(0, 8)) {
        const sev = r.severity ? `[${r.severity}] ` : "";
        const status = r.status || "open";
        context += `    • ${sev}${r.title} (${status}`;
        if (r.owner) context += `, owner: ${r.owner}`;
        if (r.dueDate) context += `, due: ${r.dueDate}`;
        context += `)`;
        if (r.description) context += `: ${r.description.substring(0, 140)}`;
        context += `\n`;
      }
      if (items.length > 8) context += `    ... and ${items.length - 8} more\n`;
    }
  }
  ...
  try {
    ...
    const hasBudget = (budgetSummary.originalContract || 0) + (budgetSummary.totalActualSpend || 0) + (budgetSummary.totalChangeOrders || 0) + (budgetSummary.totalAdditionalFunding || 0) > 0;
    if (hasBudget) {
      context += `\nBUDGET TRACKING:\n`;
      context += `- Original Contract: $${(budgetSummary.originalContract || 0).toLocaleString()}\n`;
      context += `- Change Orders: $${(budgetSummary.totalChangeOrders || 0).toLocaleString()}\n`;
      context += `- Additional Funding: $${(budgetSummary.totalAdditionalFunding || 0).toLocaleString()}\n`;
      context += `- Actual Spend: $${(budgetSummary.totalActualSpend || 0).toLocaleString()}\n`;
      context += `- Variance: $${(budgetSummary.variance || 0).toLocaleString()}\n`;
    }
  } catch {}

  // Schedule tracking — milestones, slips
  const scheduleEntries = storage.getScheduleEntries(projectId);
  if (scheduleEntries.length > 0) {
    context += `\nSCHEDULE TRACKING (${scheduleEntries.length} milestones):\n`;
    for (const s of scheduleEntries.slice(0, 15)) {
      context += `- ${s.milestone}: ${s.status || "unknown"}`;
      if (s.originalDate) context += ` (orig: ${s.originalDate}`;
      if (s.currentDate && s.currentDate !== s.originalDate) context += ` → current: ${s.currentDate}`;
      if (s.originalDate) context += `)`;
      if (s.varianceDays) context += ` — ${s.varianceDays} days variance`;
      context += `\n`;
    }
    if (scheduleEntries.length > 15) context += `  ... and ${scheduleEntries.length - 15} more milestones\n`;
  }

  // Go-Live readiness scorecard (latest across project's contract baselines)
  try {
    const contractBaselines = storage.getContractBaselines(projectId);
    let latestScorecard: any = null;
    for (const cb of contractBaselines) {
      const sc = storage.getGoLiveScorecard(cb.id);
      if (sc && (!latestScorecard || sc.assessedAt > latestScorecard.assessedAt)) {
        latestScorecard = sc;
      }
    }
    if (latestScorecard) {
      context += `\nGO-LIVE READINESS SCORECARD (assessed ${latestScorecard.assessedAt}):\n`;
      context += `- Overall Score: ${latestScorecard.overallScore ?? "N/A"}/100\n`;
      context += `- Readiness: ${latestScorecard.overallReadiness || "N/A"}\n`;
      if (latestScorecard.assessorNotes) context += `- Assessor Notes: ${latestScorecard.assessorNotes.substring(0, 400)}\n`;
      try {
        const criteria = typeof latestScorecard.criteria === "string" ? JSON.parse(latestScorecard.criteria) : latestScorecard.criteria;
        if (Array.isArray(criteria) && criteria.length > 0) {
          context += `- Criteria (${criteria.length} total):\n`;
          for (const c of criteria) {
            const label = c.name || c.key || c.criterion || "criterion";
            const score = c.score != null ? `${c.score}/10` : "unrated";
            const cat = c.category ? ` [${c.category}]` : "";
            context += `    • ${label}${cat}: ${score}`;
            if (c.notes) context += ` — ${String(c.notes).substring(0, 140)}`;
            context += `\n`;
          }
        }
      } catch {}
  ...
  // Project documents (just names/types so AI knows what's been uploaded)
  try {
    const docs = storage.getProjectDocuments(projectId);
    if (docs.length > 0) {
      context += `\nUPLOADED PROJECT DOCUMENTS (${docs.length}):\n`;
      for (const d of docs.slice(0, 20)) {
        context += `- ${d.fileName}`;
        if (d.documentType) context += ` (${d.documentType})`;
        context += `\n`;
      }
      if (docs.length > 20) context += `  ... and ${docs.length - 20} more documents\n`;
    }
  } catch {}
...
export async function assessGoLiveReadiness(data: {
  ...
  const criteriaDefs = [
    { category: "Testing", key: "sit_completion", name: "SIT Completion", weight: 10, description: "Has System Integration Testing been completed? What % of test cases passed?" },
    { category: "Testing", key: "e2e_testing", name: "E2E Testing Exit", weight: 12, description: "Have end-to-end business process tests been completed and signed off?" },
    { category: "Testing", key: "uat_completion", name: "UAT/UER Completion", weight: 12, description: "Has User Acceptance Testing been completed with sign-off from business owners?" },
    { category: "Testing", key: "payroll_compare", name: "Payroll Compare", weight: 8, description: "Have parallel payroll runs been completed and reconciled?" },
    { category: "Defects", key: "critical_high_resolution", name: "Critical/High Defect Resolution", weight: 15, description: "Have all critical and high-severity defects been resolved?" },
    { category: "Defects", key: "burndown_trend", name: "Defect Burn-down Trend", weight: 5, description: "Is the defect discovery/resolution trend positive?" },
    { category: "Data", key: "migration_quality", name: "Data Migration Quality", weight: 10, description: "Has data migration been validated with acceptable error rates?" },
    { category: "Data", key: "reconciliation", name: "Reconciliation Results", weight: 5, description: "Have financial and data reconciliation checks passed?" },
    { category: "Cutover", key: "plan_completeness", name: "Cutover Plan Completeness", weight: 8, description: "Is the cutover plan documented, reviewed, and rehearsed?" },
    { category: "Cutover", key: "rollback_plan", name: "Rollback Plan", weight: 3, description: "Is a rollback plan documented and tested?" },
    { category: "Readiness", key: "training", name: "Training Completion", weight: 5, description: "Have all end users been trained?" },
    { category: "Readiness", key: "support_model", name: "Support Model Activated", weight: 4, description: "Is the post-go-live support model (hypercare team, help desk) ready?" },
    { category: "Readiness", key: "hypercare_plan", name: "Hypercare Plan", weight: 3, description: "Is the hypercare plan documented with escalation procedures?" },
  ];
